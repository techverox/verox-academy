  ...
  return onSnapshot(q, 
    async (snapshot) => {
      try {
        ...
        console.error("[FIRESTORE] Error enriching enrollments:", err);
        callback([]);
      }
    },
    (error) => {
...
export const getAdminRevenueAnalytics = async () => {
  try {
    ...
    // Attempting query - might require index
    const q = query(
      paymentsRef,
    ...
    // If no data, return empty array to trigger mock data in UI if needed, 
    // or handle gracefully.
    return Object.entries(dailyRevenue)
      .map(([name, revenue]) => ({ name, revenue }))
      .sort((a, b) => a.name.localeCompare(b.name));
  } catch (error) {
    console.error("[FIRESTORE] Admin Revenue Analytics error:", error);
    return [];
  }
};
